"""YAML serialization for pensive state-machine models."""
...
def model_from_yaml(text: str, *, validate: bool = True) -> StateMachine:
    """Reconstruct a pensive model from YAML text."""
    loaded = yaml.safe_load(text)
    if not isinstance(loaded, dict):
        raise TypeError("pensive model YAML must load to a mapping")
...
def read_yaml(path: str | Path, *, validate: bool = True) -> StateMachine:
    """Read a pensive model from a YAML file."""
...
def model_from_dict(data: Mapping[str, Any], *, validate: bool = True) -> StateMachine:
    """Reconstruct a pensive model from a decoded YAML mapping."""
    if data.get("schema") != SCHEMA:
        raise ValueError(f"unsupported pensive YAML schema {data.get('schema')!r}")
    if data.get("version") != VERSION:
        raise ValueError(f"unsupported pensive YAML version {data.get('version')!r}")
    class_path = data.get("class")
    if not isinstance(class_path, str):
        raise TypeError("pensive model YAML requires a string class path")
    spec = _registry_by_path().get(class_path)
    if spec is None:
        raise ValueError(f"unregistered pensive model class {class_path!r}")

    graph = _graph_from_data(data.get("graph", {}), validate=validate)
    metadata = _decode(data.get("metadata", {}), validate=validate)
    if not isinstance(metadata, dict):
        raise TypeError("pensive model metadata must decode to a mapping")
...
def _graph_from_data(data: Any, *, validate: bool = True) -> TransitionGraph:
    if not isinstance(data, Mapping):
        raise TypeError("pensive model graph must be a mapping")
...
def _decode(value: Any, *, validate: bool = True) -> Any:
    ...
    raise ValueError(f"unknown pensive YAML value tag {tag!r}")
