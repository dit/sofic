...
class NestedWord:
    ...
    def validate(self) -> None:
        ...
        if len(self.symbols) != len(self.kinds) or len(self.symbols) != len(self.matching):
            raise PensiveValidationError("symbols, kinds, and matching must have the same length")

        for index, kind in enumerate(self.kinds):
            if kind not in _KINDS:
                raise PensiveValidationError(f"invalid nested-word kind {kind!r} at position {index}")
        ...
        for index, (kind, partner) in enumerate(zip(self.kinds, self.matching, strict=True)):
            ...
            if not isinstance(partner, int) or partner < 0 or partner >= len(self.symbols):
                raise PensiveValidationError(f"matching partner {partner!r} out of range at position {index}")
            if self.matching[partner] != index:
                raise PensiveValidationError("matching relation must be symmetric")
            partner_kind = self.kinds[partner]
            if kind == KIND_INTERNAL:
                raise PensiveValidationError("internal positions cannot be matched")
            if kind == KIND_CALL and not (partner_kind == KIND_RETURN and index < partner):
                raise PensiveValidationError("call positions must match later return positions")
            if kind == KIND_RETURN and not (partner_kind == KIND_CALL and partner < index):
                raise PensiveValidationError("return positions must match earlier call positions")
        ...
        for index, (kind, partner) in enumerate(zip(self.kinds, self.matching, strict=True)):
            if kind == KIND_CALL and partner is not None:
                ...
                if not stack or stack[-1] != partner:
                    raise PensiveValidationError("matching relation must be properly nested")
                stack.pop()
...
class NestedWordAutomaton(StateMachine):
    ...
    def recognizes(self, word: NestedWord) -> bool:
        from pensive.automata.nwa_simulation import recognizes_nwa
