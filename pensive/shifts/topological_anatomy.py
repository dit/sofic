...
2. Build the Parry MME as a labeled :class:`~pensive.generators.mealy.MealyHMM`
   (:func:`parry_measure_sofic`).
3. Minimize to the causal presentation
   (:meth:`~pensive.generators.epsilon_machine.EpsilonMachine.from_hmm`), pair it
   with its time reverse
   (:meth:`~pensive.generators.epsilon_machine.EpsilonMachine.to_bidirectional`),
   and read the anatomy
   (:meth:`~pensive.generators.bidirectional_epsilon_machine.BidirectionalEpsilonMachine.information_anatomy`).
...
def _right_resolving(shift: SoficShift) -> SoficShift:
    ...
    Fischer cover (:meth:`~pensive.shifts.covers.RightFischerCover.from_sofic`) is
    ...
    unifilar a :class:`~pensive.exceptions.UnifilarityError` is raised asking for a
    right-resolving input.
    """
    if shift.is_unifilar():
        return shift
    from pensive.shifts.covers import RightFischerCover
...
def parry_measure_sofic(shift: SoficShift) -> MealyHMM:
    ...
    process.
    """
    from pensive.shifts.parry_construction import parry_measure
