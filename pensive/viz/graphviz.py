"""Graphviz rendering for pensive state-machine models."""
...
def _require_graphviz() -> Any:
    try:
        ...
        raise ImportError(
            "Graphviz rendering requires the optional pensive[viz] extra "
            "(pip install 'pensive[viz]') and the Graphviz system binaries."
        ) from exc
    return graphviz
...
def _node_name(state: Hashable) -> str:
    ...
    if isinstance(state, MixedState):
        text = format_belief(state.belief)
    else:
