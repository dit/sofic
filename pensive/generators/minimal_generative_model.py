...
class MinimalGenerativeModel(MealyHMM):
    ...
    def __init__(
        ...
        *,
        ...
        exact_common_information: float = 0.0,
...
def minimal_generative_model(
    ...
    *,
    ...
    pair_channel, exact_common_information = _generative_state_channel(
        joint,
        bound=bound,
        niter=niter,
        maxiter=maxiter,
    ...
    return _model_from_channel(
        bidir,
        joint,
        pair_channel,
        exact_common_information=exact_common_information,
        cutoff=cutoff,
    )
...
def _generative_state_channel(
    ...
    *,
    ...
    opt = _optimize_exact_common_information(
        dist,
        bound=bound,
        niter=niter,
        maxiter=maxiter,
    ...
    if aux_joint.ndim < 3:
        raise StochasticValidationError("exact common information optimizer returned an invalid joint shape")
    if aux_joint.ndim > 3:
    ...
    exact_common_information = float(opt.objective(opt._optima))
    return channel, exact_common_information
...
def _model_from_channel(
    ...
    *,
    ...
    if total_state_mass <= 0.0:
        raise StochasticValidationError("minimal generative model has empty state support")
    ...
    for source in active_states:
        ...
        if total <= cutoff:
            raise StochasticValidationError(f"minimal generative model state {source!r} has no outgoing mass")
...
__all__ = ["MinimalGenerativeModel", "minimal_generative_model"]
